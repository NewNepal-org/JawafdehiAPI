    ...
    for rel_str, record in files_dict.items():
        ...
        if local_path.is_file():
            try:
                ...
                logger.debug("Could not verify checksum for %s, re-downloading: %s", rel_str, exc)
        ...
        try:
            ...
            logger.warning("Failed to download workflow output %s: %s", backend_path, exc)
...
def record_downloaded_files(
    ...
    *progress_data* is mutated in place AND returned so callers can chain
    ...
    for abs_path in local_paths:
        if not abs_path.is_file():
            logger.warning("record_downloaded_files: not a file, skipping: %s", abs_path)
            continue
