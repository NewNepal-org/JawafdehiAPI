...
def test_moderators_can_transition_to_any_state(case_data, moderator_data, target_state):
    ...
    assert can_transition, f"Moderator should be able to transition case to {target_state} state"
...
def test_transition_to_in_review_updates_version_info(case_data):
    ...
    assert case.versionInfo is not None, "versionInfo should be updated after state transition"
...
def test_contributor_cannot_publish_case():
    ...
    assert not can_publish, "Contributor should NOT be able to transition case to PUBLISHED state"
...
def test_version_info_contains_version_number():
    ...
    assert "version_number" in case.versionInfo, "versionInfo should contain version_number field"
