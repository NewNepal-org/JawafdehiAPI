...
class TestPublicAPIWorkflows:
    ...
    def setup_method(self):
        ...
        self.published_corruption_case = create_case_with_entities(
            ...
            key_allegations=["Illegally acquired public land", "Failed to disclose assets"],
    ...
    def test_audit_history_retrieval(self):
        ...
        assert "audit_history" in case_detail, "Detail endpoint should include audit_history"

        audit_history = case_detail["audit_history"]
        assert len(audit_history) == 2, "Audit history should include both published versions"

        # Verify audit history is in reverse chronological order (newest first)
        assert audit_history[0]["version_number"] == 2, "First entry should be version 2 (newest)"
        assert audit_history[1]["version_number"] == 1, "Second entry should be version 1"
    ...
    def test_search_across_multiple_fields(self):
        ...
        found_promises_case = any(
            case["title"] == "Broken Promise - Infrastructure Project" for case in results
        )
    ...
    def test_document_source_visibility_workflow(self):
        ...
        draft_source = create_document_source_with_entities(
            title="Draft Source - Should Not Appear", description="Source for draft case"
        )

        # Add evidence to draft case referencing this source
        self.draft_case.evidence = [
            {"source_id": draft_source.source_id, "description": "Evidence from draft case"}
        ]
        ...
        assert draft_source.source_id not in source_ids, "Source from draft case should NOT appear"
        ...
        assert response.status_code == 404, "Source from draft case should not be accessible"
