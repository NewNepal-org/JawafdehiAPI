...
def create_user_with_role(username, email, role, password="testpass123"):
    ...
    view_perm, _ = Permission.objects.get_or_create(
        codename="view_case", content_type=content_type, defaults={"name": "Can view case"}
    )
    change_perm, _ = Permission.objects.get_or_create(
        codename="change_case", content_type=content_type, defaults={"name": "Can change case"}
    )
    add_perm, _ = Permission.objects.get_or_create(
        codename="add_case", content_type=content_type, defaults={"name": "Can add case"}
    )
    delete_perm, _ = Permission.objects.get_or_create(
        codename="delete_case", content_type=content_type, defaults={"name": "Can delete case"}
    )

    # Get or create User permissions (for moderators to manage users)
    user_view_perm, _ = Permission.objects.get_or_create(
        codename="view_user", content_type=user_content_type, defaults={"name": "Can view user"}
    )
    user_change_perm, _ = Permission.objects.get_or_create(
        codename="change_user", content_type=user_content_type, defaults={"name": "Can change user"}
    )
    user_add_perm, _ = Permission.objects.get_or_create(
        codename="add_user", content_type=user_content_type, defaults={"name": "Can add user"}
    )
    user_delete_perm, _ = Permission.objects.get_or_create(
        codename="delete_user", content_type=user_content_type, defaults={"name": "Can delete user"}
    )
