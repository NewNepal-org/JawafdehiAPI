...
from tests.conftest import create_case_with_entities, create_mock_request, create_user_with_role
from tests.strategies import simple_complete_case_data as complete_case_data
from tests.strategies import user_with_role
...
def test_contributors_can_transition_between_draft_and_in_review(
    ...
    contributor = create_user_with_role(
        contributor_data["username"], contributor_data["email"], contributor_data["role"]
    )
...
def test_contributors_cannot_transition_to_published_or_closed(
    ...
    assert (
        "Contributors can only transition between DRAFT and IN_REVIEW" in error_message
...
def test_contributor_can_only_access_assigned_cases(case_data, contributor_data):
    ...
    assert has_permission_assigned, "Contributor should have change permission for assigned case"
...
def test_contributor_cannot_modify_unassigned_cases(case_data, contributor_data):
    ...
    has_permission = admin.has_change_permission(request, case)
    assert not has_permission, "Contributor should NOT have change permission for unassigned case"
