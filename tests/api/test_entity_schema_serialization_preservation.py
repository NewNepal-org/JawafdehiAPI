...
def test_preservation_case_metadata_fields():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    assert isinstance(case_data["id"], int), f"id should be integer, got {type(case_data['id'])}: {case_data['id']}"
    assert case_data["id"] == case.id, f"id should match case.id ({case.id}), got {case_data['id']}"
    
    assert "case_id" in case_data, "Response should include 'case_id' field"
    assert isinstance(case_data["case_id"], str), f"case_id should be string, got {type(case_data['case_id'])}: {case_data['case_id']}"
    assert case_data["case_id"] == case.case_id, f"case_id should match case.case_id ({case.case_id}), got {case_data['case_id']}"
    
    assert "title" in case_data, "Response should include 'title' field"
    assert isinstance(case_data["title"], str), f"title should be string, got {type(case_data['title'])}: {case_data['title']}"
    assert case_data["title"] == case.title, f"title should match case.title ({case.title}), got {case_data['title']}"
    
    assert "case_type" in case_data, "Response should include 'case_type' field"
    assert isinstance(case_data["case_type"], str), f"case_type should be string, got {type(case_data['case_type'])}: {case_data['case_type']}"
    assert case_data["case_type"] == case.case_type.value, f"case_type should match case.case_type ({case.case_type.value}), got {case_data['case_type']}"
    
    assert "state" in case_data, "Response should include 'state' field"
    assert isinstance(case_data["state"], str), f"state should be string, got {type(case_data['state'])}: {case_data['state']}"
    assert case_data["state"] == case.state.value, f"state should match case.state ({case.state.value}), got {case_data['state']}"
    
    assert "description" in case_data, "Response should include 'description' field"
    assert isinstance(case_data["description"], str), f"description should be string, got {type(case_data['description'])}: {case_data['description']}"
    assert case_data["description"] == case.description, f"description should match case.description, got {case_data['description']}"
    
    assert "key_allegations" in case_data, "Response should include 'key_allegations' field"
    assert isinstance(case_data["key_allegations"], list), f"key_allegations should be list, got {type(case_data['key_allegations'])}: {case_data['key_allegations']}"
    assert case_data["key_allegations"] == case.key_allegations, f"key_allegations should match case.key_allegations, got {case_data['key_allegations']}"
...
def test_preservation_timestamp_fields():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    assert isinstance(case_data["created_at"], str), f"created_at should be string, got {type(case_data['created_at'])}: {case_data['created_at']}"
    # Verify it's a valid ISO datetime string format
    from datetime import datetime
    try:
        datetime.fromisoformat(case_data["created_at"].replace('Z', '+00:00'))
    except ValueError:
        assert False, f"created_at should be valid ISO datetime string, got {case_data['created_at']}"
    
    assert "updated_at" in case_data, "Response should include 'updated_at' field"
    assert isinstance(case_data["updated_at"], str), f"updated_at should be string, got {type(case_data['updated_at'])}: {case_data['updated_at']}"
    # Verify it's a valid ISO datetime string format
    try:
        datetime.fromisoformat(case_data["updated_at"].replace('Z', '+00:00'))
    except ValueError:
        assert False, f"updated_at should be valid ISO datetime string, got {case_data['updated_at']}"
...
def test_preservation_array_fields():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    assert isinstance(case_data["timeline"], list), f"timeline should be list, got {type(case_data['timeline'])}: {case_data['timeline']}"
    ...
    assert isinstance(case_data["evidence"], list), f"evidence should be list, got {type(case_data['evidence'])}: {case_data['evidence']}"
    ...
    assert isinstance(case_data["tags"], list), f"tags should be list, got {type(case_data['tags'])}: {case_data['tags']}"
    
    # key_allegations should be preserved as tested in metadata test
    assert "key_allegations" in case_data, "Response should include 'key_allegations' field"
    assert isinstance(case_data["key_allegations"], list), f"key_allegations should be list, got {type(case_data['key_allegations'])}: {case_data['key_allegations']}"
    assert len(case_data["key_allegations"]) == 2, f"key_allegations should have 2 items, got {len(case_data['key_allegations'])}: {case_data['key_allegations']}"
...
def test_preservation_json_fields():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    assert isinstance(version_info, (dict, type(None))), f"versionInfo should be dict or null, got {type(version_info)}: {version_info}"
    
    # Test other optional fields that might be JSON-like
    assert "notes" in case_data, "Response should include 'notes' field"
    notes = case_data["notes"]
    assert isinstance(notes, (str, type(None))), f"notes should be string or null, got {type(notes)}: {notes}"
...
def test_preservation_optional_fields():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    optional_fields = [
        "short_description", "thumbnail_url", "banner_url", 
        "case_start_date", "case_end_date", "notes"
    ]
    ...
    for field in optional_fields:
        ...
        assert isinstance(field_value, (str, type(None))), f"Optional field '{field}' should be string or null, got {type(field_value)}: {field_value}"
...
def test_preservation_property_based_non_entity_fields(case_data):
    ...
    **Validates: Requirements 3.1, 3.2, 3.3**
    ...
    for field_name, expected_type in core_fields.items():
        assert field_name in case_response, f"Response should include core field '{field_name}'"
        field_value = case_response[field_name]
        assert isinstance(field_value, expected_type), f"Field '{field_name}' should be {expected_type.__name__}, got {type(field_value).__name__}: {field_value}"
    
    # Test preservation of optional fields (can be present and null/empty)
    optional_fields = [
        "short_description", "thumbnail_url", "banner_url",
        "case_start_date", "case_end_date", "notes", "versionInfo"
    ]
    
    for field_name in optional_fields:
        assert field_name in case_response, f"Response should include optional field '{field_name}'"
    ...
    assert isinstance(case_response, dict), f"Response should be dict, got {type(case_response)}"
    
    # Verify that array fields are actually arrays
    array_fields = ["key_allegations", "timeline", "evidence", "tags"]
    for field_name in array_fields:
        field_value = case_response[field_name]
        assert isinstance(field_value, list), f"Array field '{field_name}' should be list, got {type(field_value)}: {field_value}"
...
def test_preservation_response_structure_consistency():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    assert isinstance(case_data, dict), f"Response should be dict, got {type(case_data)}"
    
    # Test that response has a reasonable number of fields (not too few, not too many)
    field_count = len(case_data.keys())
    assert field_count >= 15, f"Response should have at least 15 fields, got {field_count}: {list(case_data.keys())}"
    assert field_count <= 30, f"Response should have at most 30 fields, got {field_count}: {list(case_data.keys())}"
    
    # Test that all field names are strings
    for field_name in case_data.keys():
        assert isinstance(field_name, str), f"Field name should be string, got {type(field_name)}: {field_name}"
    ...
    for field_name, field_value in case_data.items():
        ...
        if isinstance(field_value, dict):
            ...
            assert len(field_value) <= 10, f"Nested dict '{field_name}' should be simple, got {len(field_value)} keys: {field_value}"
            ...
            if len(field_value) > 0:
                ...
                if isinstance(first_item, dict):
                    ...
                    assert len(first_item) <= 10, f"Array item in '{field_name}' should be simple dict, got {len(first_item)} keys: {first_item}"
...
def test_preservation_case_with_entities_non_entity_fields():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    assert case_data["title"] == case.title, f"Title should match case.title, got {case_data['title']}"
    
    assert "description" in case_data, "Description should be present even when entities exist"
    assert case_data["description"] == case.description, f"Description should match case.description"
    
    assert "key_allegations" in case_data, "Key allegations should be present even when entities exist"
    assert case_data["key_allegations"] == case.key_allegations, f"Key allegations should match case.key_allegations"
    
    assert "case_type" in case_data, "Case type should be present even when entities exist"
    assert case_data["case_type"] == case.case_type.value, f"Case type should match case.case_type"
    
    assert "state" in case_data, "State should be present even when entities exist"
    assert case_data["state"] == case.state.value, f"State should match case.state"
