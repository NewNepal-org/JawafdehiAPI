...
class TestStatisticsCounting:
    ...
    def test_published_cases_count(self, api_client):
        ...
        Case.objects.create(
            case_type=CaseType.CORRUPTION, state=CaseState.PUBLISHED, title="Published Case 1"
        )
        Case.objects.create(
            case_type=CaseType.CORRUPTION, state=CaseState.PUBLISHED, title="Published Case 2"
        )
        Case.objects.create(
...
class TestStatisticsCaching:
    ...
    def test_statistics_are_cached(self, api_client):
        ...
        # Create initial case
        Case.objects.create(
            case_type=CaseType.CORRUPTION, state=CaseState.PUBLISHED, title="Initial Case"
        )
...
class TestStatisticsPerformance:
    ...
    def test_statistics_with_large_dataset(self, api_client):
        ...
        for i in range(3):
            Case.objects.create(
                case_type=CaseType.CORRUPTION, state=CaseState.DRAFT, title=f"Draft Case {i}"
            )

        for i in range(2):
            Case.objects.create(
                case_type=CaseType.CORRUPTION, state=CaseState.CLOSED, title=f"Closed Case {i}"
            )
