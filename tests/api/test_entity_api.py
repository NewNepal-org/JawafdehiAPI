...
def valid_entity_id(draw):
    ...
    slug = draw(
        st.text(alphabet="abcdefghijklmnopqrstuvwxyz0123456789-", min_size=3, max_size=50).filter(
...
def test_entity_list_includes_all_entities():
    ...
    case = Case.objects.create(
        case_id="test-case", state=CaseState.PUBLISHED, title="Test Case", description="Test"
    )
...
def test_entity_retrieve_with_display_name():
    ...
    entity = JawafEntity.objects.create(nes_id="entity:person/john-doe", display_name="John Doe")
...
def test_entity_retrieve_response_structure():
    ...
    entity = JawafEntity.objects.create(nes_id="entity:person/test", display_name="Test Person")
