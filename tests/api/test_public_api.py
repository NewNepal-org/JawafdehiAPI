...
def test_public_api_only_shows_published_cases(case_data, state):
    ...
    assert response.status_code == 200, f"API should return 200 OK, but got {response.status_code}"
...
def test_evidence_requires_valid_source_references(case_data, source_data):
    ...
    case.evidence = [
        {"source_id": source.source_id, "description": "This source supports the allegation"}
    ]
    case.save()
    ...
    assert len(evidence_list) > 0, "Published case should include evidence in API response"
...
def test_evidence_with_invalid_source_reference():
    ...
    case.evidence = [
        {"source_id": "source:nonexistent:12345678", "description": "Invalid source reference"}
    ]
...
@given(
    case_data=complete_case_data(),
    search_term=st.text(
        min_size=3, max_size=20, alphabet=st.characters(whitelist_categories=("Ll", "Lu"))
    ),
)
...
def test_filter_by_tags(case_data, tag):
    ...
    for returned_case in response.data.get("results", []):
        ...
        assert tag in returned_tags, f"Filtered results should only include cases with tag '{tag}'"
...
def test_published_cases_display_complete_data(case_data, source_data):
    ...
    # Add evidence referencing the source
    case.evidence = [{"source_id": source.source_id, "description": "Evidence description"}]
    case.save()
...
def test_published_cases_include_all_entity_fields(case_data):
    ...
    assert "alleged_entities" in returned_case, "Response should include alleged_entities"
    assert "related_entities" in returned_case, "Response should include related_entities"
    ...
    assert isinstance(returned_case["alleged_entities"], list), "alleged_entities should be a list"
    assert (
...
def test_api_does_not_expose_contributors():
    ...
    assert "contributors" not in response.data, "API should not expose contributors field"
...
def test_document_source_api_only_shows_sources_referenced_by_published_cases():
    ...
    create_case_with_entities(
        ...
        case_type=CaseType.CORRUPTION,
        state=CaseState.DRAFT,
        evidence=[{"source_id": draft_source.source_id, "description": "Evidence from draft case"}],
    )
    ...
    create_case_with_entities(
        ...
        evidence=[
            {"source_id": in_review_source.source_id, "description": "Evidence from in-review case"}
        ],
    )
    ...
    create_case_with_entities(
        ...
        state=CaseState.PUBLISHED,
        evidence=[
            {"source_id": published_source.source_id, "description": "Evidence from published case"}
        ],
    )
