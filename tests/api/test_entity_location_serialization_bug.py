...
def test_bug_condition_missing_legacy_entity_fields():
    ...
    **EXPECTED OUTCOME**: Test FAILS (this is correct - it proves the bug exists)
    ...
    # The bug is that the unified entities field structure is incorrect

    # Test that unified entities field is present
    assert "entities" in case_data, "Response should include unified entities field"

    # Verify entities field is an array
    entities = case_data["entities"]

    assert isinstance(
        entities, list
    ), f"entities should be a list, but got {type(entities).__name__}: {entities}"

    # Verify we have the expected entities (2 non-location entities: alleged person + related org)
    assert (
        len(entities) == 2
    ), f"Should have 2 entities, got {len(entities)}: {entities}"

    # Find the alleged entity (person)
    alleged_entity = next((e for e in entities if e.get("type") == "alleged"), None)
    assert alleged_entity is not None, f"Should have an alleged entity in: {entities}"

    # Verify entity structure in unified field
    assert isinstance(
        alleged_entity, dict
    ), f"Alleged entity should be a dict, but got {type(alleged_entity).__name__}: {alleged_entity}"
    assert (
        "id" in alleged_entity
    ), f"Alleged entity should have 'id' field: {alleged_entity}"
    assert (
        "nes_id" in alleged_entity
    ), f"Alleged entity should have 'nes_id' field: {alleged_entity}"
    assert (
        "display_name" in alleged_entity
    ), f"Alleged entity should have 'display_name' field: {alleged_entity}"
    assert (
        "type" in alleged_entity
    ), f"Alleged entity should have 'type' field: {alleged_entity}"
    assert (
        "notes" in alleged_entity
    ), f"Alleged entity should have 'notes' field: {alleged_entity}"

    # Verify the entity data matches
    assert (
        alleged_entity["nes_id"] == entity_person.nes_id
    ), f"Alleged entity nes_id should match: expected {entity_person.nes_id}, got {alleged_entity['nes_id']}"
    assert (
        alleged_entity["type"] == "alleged"
    ), f"Entity type should be 'alleged', got {alleged_entity['type']}"
...
def test_bug_condition_wrong_entity_id_in_unified_fields():
    ...
    **EXPECTED OUTCOME**: Test FAILS (this is correct - it proves the bug exists)
    ...
    _ = CaseEntityRelationship.objects.create(
        case=case,
        entity=entity_person,
        relationship_type=RelationshipType.ALLEGED,
    ...
    assert (
        entity["id"] == entity_person.id
    ), f"Entity id should be entity.id ({entity_person.id}), but got relationship.id ({entity['id']})"
    assert (
        entity["nes_id"] == entity_person.nes_id
    ), f"Entity nes_id should match: expected {entity_person.nes_id}, got {entity['nes_id']}"
...
def test_bug_condition_property_based_entities_serialization(case_data):
    ...
    **Validates: Requirements 1.1, 1.2, 1.3**
    ...
    assert isinstance(
        detail_data["entities"], list
    ), f"entities should be list, got {type(detail_data['entities']).__name__}: {detail_data['entities']}"

    assert "locations" in detail_data, "Detail response should include locations field"
    assert isinstance(
        detail_data["locations"], list
    ), f"locations should be list, got {type(detail_data['locations']).__name__}: {detail_data['locations']}"
    ...
    if (
        ...
        assert isinstance(
            our_case["entities"], list
        ), f"entities should be list, got {type(our_case['entities']).__name__}: {our_case['entities']}"

        assert "locations" in our_case, "List response should include locations field"
        assert isinstance(
            our_case["locations"], list
        ), f"locations should be list, got {type(our_case['locations']).__name__}: {our_case['locations']}"
...
def test_bug_condition_empty_entities_case():
    ...
    **EXPECTED OUTCOME**: Test FAILS (this is correct - it proves the bug exists)
    ...
    assert (
        case.entity_relationships.count() == 0
    ), "Case should have no entity relationships"
    ...
    assert isinstance(
        entities, list
    ), f"entities should be empty list, got {type(entities).__name__}: {entities}"
    ...
    assert isinstance(
        locations, list
    ), f"locations should be empty list, got {type(locations).__name__}: {locations}"
...
def test_bug_condition_mixed_entity_types():
    ...
    **EXPECTED OUTCOME**: Test FAILS (this is correct - it proves the bug exists)
    ...
    assert isinstance(
        entities, list
    ), f"entities should be list, got {type(entities).__name__}: {entities}"
    assert (
        len(entities) >= 3
    ), f"Should have at least 3 non-location entities, got {len(entities)}: {entities}"
    ...
    for expected_type in expected_types:
        assert (
            expected_type in entity_types
        ), f"Should include entity with type '{expected_type}', got types: {entity_types}"
    ...
    assert isinstance(
        locations, list
    ), f"locations should be list, got {type(locations).__name__}: {locations}"
    ...
    if len(locations) > 0:
        for location in locations:
            assert isinstance(
                location, dict
            ), f"Location should be dict, got {type(location).__name__}: {location}"
            assert location.get("nes_id", "").startswith(
                "entity:location/"
            ), f"Location should have location nes_id: {location}"
