...
def test_bug_condition_unified_entities_schema_compliance():
    ...
    **EXPECTED OUTCOME**: Test FAILS (this is correct - it proves the bug exists)
    ...
    entity_location = create_entities_from_ids(["entity:location/district/kathmandu"])[
        0
    ]
    ...
    assert (
        "entities" in case_data
    ), "Response should include unified 'entities' array as primary entity field"
    ...
    assert (
        len(entities) >= 2
    ), f"Should have at least 2 entities (person + org), got {len(entities)}: {entities}"

    for entity in entities:
        assert isinstance(
            entity, dict
        ), f"Entity should be a dict, but got {type(entity).__name__}: {entity}"
        ...
        assert (
            not missing_fields
        ), f"Entity missing required fields {missing_fields}: {entity}"
        ...
        ), f"Entity should not include internal relationship metadata {forbidden_present}: {entity}"

        # Verify field types and values
        assert isinstance(
            entity["id"], int
        ), f"Entity id should be integer, got {type(entity['id'])}: {entity['id']}"
        assert isinstance(
            entity["nes_id"], (str, type(None))
        ), f"Entity nes_id should be string or null, got {type(entity['nes_id'])}: {entity['nes_id']}"
        assert isinstance(
            entity["display_name"], (str, type(None))
        ), f"Entity display_name should be string or null, got {type(entity['display_name'])}: {entity['display_name']}"
        assert isinstance(
            entity["type"], str
        ), f"Entity type should be string, got {type(entity['type'])}: {entity['type']}"
        assert isinstance(
            entity["notes"], (str, type(None))
        ), f"Entity notes should be string or null, got {type(entity['notes'])}: {entity['notes']}"
    ...
    for field in complex_fields:
        if field in case_data:
            print(
                f"🐛 BUG DETECTED: Legacy field '{field}' is present alongside unified format"
            )
            field_data = case_data[field]
            ...
            assert (
                False
            ), f"Schema discrepancy: Both unified 'entities' and legacy '{field}' are present. API should use unified format only."
...
def test_bug_condition_entity_id_mapping_correctness():
    ...
    **EXPECTED OUTCOME**: Test FAILS (this is correct - it proves the bug exists)
    ...
    dummy_entities = create_entities_from_ids(
        ["entity:person/dummy-1", "entity:person/dummy-2"]
    )
    ...
    assert (
        len(entities) >= 1
    ), f"Should have at least 1 entity, got {len(entities)}: {entities}"
    ...
    assert (
        test_entity is not None
    ), f"Could not find test entity with nes_id {entity_person.nes_id} in entities: {entities}"

    # **BUG CONDITION**: This will FAIL on unfixed code
    # The id field should be the entity ID, not the relationship ID
    assert (
        test_entity["id"] == entity_person.id
    ), f"Entity id should be entity.id ({entity_person.id}), but got {test_entity['id']}"
    ...
    print(
        f"DEBUG: Entity ID: {entity_person.id}, Relationship ID: {relationship.id}, Serialized ID: {test_entity['id']}"
    )

    # The key test is that we get the entity ID, regardless of whether it matches relationship ID
    assert isinstance(
        test_entity["id"], int
    ), f"Entity id should be integer, got {type(test_entity['id'])}"
    assert (
        test_entity["id"] > 0
    ), f"Entity id should be positive, got {test_entity['id']}"
...
def test_bug_condition_no_internal_relationship_metadata():
    ...
    **EXPECTED OUTCOME**: Test FAILS (this is correct - it proves the bug exists)
    ...
    assert (
        len(entities) >= 2
    ), f"Should have at least 2 entities, got {len(entities)}: {entities}"
    ...
    for entity in entities:
        if isinstance(entity, dict):
            ...
            ), f"Entity should not contain internal relationship metadata {metadata_present}: {entity}"
            ...
            ), f"Entity contains unexpected fields {unexpected_fields}, should only have {expected_fields}: {entity}"
...
def test_bug_condition_property_based_unified_schema(case_data):
    ...
    **Validates: Requirements 2.1, 2.2, 2.3**
    ...
    for entity in entities:
        assert isinstance(
            entity, dict
        ), f"Entity should be dict, got {type(entity).__name__}: {entity}"
        ...
        assert (
            not forbidden_present
        ), f"Entity should not include metadata {forbidden_present}: {entity}"
...
def test_bug_condition_empty_case_unified_schema():
    ...
    **EXPECTED OUTCOME**: Test FAILS (this is correct - it proves the bug exists)
    ...
    if "alleged_entities" in case_data:
        ...
        assert isinstance(
            alleged, list
        ), f"alleged_entities should be list if present, got {type(alleged).__name__}: {alleged}"
        assert (
            len(alleged) == 0
        ), f"alleged_entities should be empty list, got: {alleged}"
