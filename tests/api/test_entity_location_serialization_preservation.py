...
def test_preservation_non_entity_fields_basic():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    ), f"key_allegations should be list, got {type(case_data['key_allegations'])}"
    assert (
        case_data["key_allegations"] == case.key_allegations
    ), f"key_allegations should match: expected {case.key_allegations}, got {case_data['key_allegations']}"

    assert "tags" in case_data, "Response should include tags field"
    assert isinstance(
        case_data["tags"], list
    ), f"tags should be list, got {type(case_data['tags'])}"
    assert (
        case_data["tags"] == case.tags
    ), f"tags should match: expected {case.tags}, got {case_data['tags']}"

    # Complex structured fields
    assert "timeline" in case_data, "Response should include timeline field"
    assert isinstance(
        case_data["timeline"], list
    ), f"timeline should be list, got {type(case_data['timeline'])}"
    assert (
        case_data["timeline"] == case.timeline
    ), f"timeline should match: expected {case.timeline}, got {case_data['timeline']}"

    assert "evidence" in case_data, "Response should include evidence field"
    assert isinstance(
        case_data["evidence"], list
    ), f"evidence should be list, got {type(case_data['evidence'])}"
    assert (
        case_data["evidence"] == case.evidence
    ), f"evidence should match: expected {case.evidence}, got {case_data['evidence']}"

    # Metadata fields
    assert "notes" in case_data, "Response should include notes field"
    assert (
        case_data["notes"] == case.notes
    ), f"notes should match: expected {case.notes}, got {case_data['notes']}"

    assert "versionInfo" in case_data, "Response should include versionInfo field"
    assert isinstance(
        case_data["versionInfo"], dict
    ), f"versionInfo should be dict, got {type(case_data['versionInfo'])}"
    assert (
        case_data["versionInfo"] == case.versionInfo
    ), f"versionInfo should match: expected {case.versionInfo}, got {case_data['versionInfo']}"
...
def test_preservation_empty_list_fields():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    assert (
        case_data["key_allegations"] == []
    ), f"key_allegations should be empty list, got {case_data['key_allegations']}"

    assert "tags" in case_data, "Response should include tags field"
    assert (
        case_data["tags"] == []
    ), f"tags should be empty list, got {case_data['tags']}"

    assert "timeline" in case_data, "Response should include timeline field"
    assert (
        case_data["timeline"] == []
    ), f"timeline should be empty list, got {case_data['timeline']}"

    assert "evidence" in case_data, "Response should include evidence field"
    assert (
        case_data["evidence"] == []
    ), f"evidence should be empty list, got {case_data['evidence']}"
...
def test_preservation_property_based_non_entity_fields(case_data):
    ...
    **Validates: Requirements 3.1, 3.2, 3.3**
    ...
    assert (
        detail_data["title"] == case.title
    ), f"title should match: expected {case.title}, got {detail_data['title']}"
    assert (
        detail_data["description"] == case.description
    ), f"description should match: expected {case.description}, got {detail_data['description']}"
    assert (
        detail_data["case_type"] == case.case_type
    ), f"case_type should match: expected {case.case_type}, got {detail_data['case_type']}"
    assert (
        detail_data["state"] == case.state
    ), f"state should match: expected {case.state}, got {detail_data['state']}"
    assert (
        detail_data["case_id"] == case.case_id
    ), f"case_id should match: expected {case.case_id}, got {detail_data['case_id']}"

    # Test list fields
    assert (
        detail_data["key_allegations"] == case.key_allegations
    ), f"key_allegations should match: expected {case.key_allegations}, got {detail_data['key_allegations']}"

    if case.tags:
        assert (
            detail_data["tags"] == case.tags
        ), f"tags should match: expected {case.tags}, got {detail_data['tags']}"

    if case.timeline:
        assert (
            detail_data["timeline"] == case.timeline
        ), f"timeline should match: expected {case.timeline}, got {detail_data['timeline']}"
    ...
    if (
        our_case
    ):  # Case might not appear in list if it doesn't meet publication criteria
        # Test that the same non-entity fields are preserved in list view
        assert (
            our_case["title"] == case.title
        ), f"List view title should match: expected {case.title}, got {our_case['title']}"
        assert (
            our_case["case_type"] == case.case_type
        ), f"List view case_type should match: expected {case.case_type}, got {our_case['case_type']}"
        assert (
            our_case["state"] == case.state
        ), f"List view state should match: expected {case.state}, got {our_case['state']}"
        assert (
            our_case["case_id"] == case.case_id
        ), f"List view case_id should match: expected {case.case_id}, got {our_case['case_id']}"
...
def test_preservation_version_info_json_field():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    assert (
        case_data["versionInfo"] == version_info
    ), f"versionInfo should match exactly: expected {version_info}, got {case_data['versionInfo']}"
...
def test_preservation_timestamp_fields():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    ), f"created_at should be string, got {type(case_data['created_at'])}"
    assert isinstance(
        case_data["updated_at"], str
    ), f"updated_at should be string, got {type(case_data['updated_at'])}"
    ...
    try:
        ...
        assert (
            abs(
                (
                    parsed_created - original_created_at.replace(tzinfo=timezone.utc)
                ).total_seconds()
            )
            < 5
        )
        assert (
            abs(
                (
                    parsed_updated - original_updated_at.replace(tzinfo=timezone.utc)
                ).total_seconds()
            )
            < 5
        )
...
def test_preservation_case_list_endpoint():
    ...
    **EXPECTED OUTCOME**: Test PASSES (confirms baseline behavior to preserve)
    ...
    for case_id in case_ids:
        assert (
            case_id in result_case_ids
        ), f"Case {case_id} should appear in list results"
