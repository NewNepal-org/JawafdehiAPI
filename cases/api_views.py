...
class CaseViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        highest_versions = published_cases.values("case_id").annotate(max_version=Max("version"))

        # Build a list of (case_id, version) tuples for filtering
        case_version_pairs = [(item["case_id"], item["max_version"]) for item in highest_versions]
        ...
        if tags_param:
            ...
            if connection.vendor == "postgresql":
                ...
                case_ids_with_tag = [
                    case.id for case in queryset if case.tags and tags_param in case.tags
                ]
...
class DocumentSourceViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return DocumentSource.objects.filter(source_id__in=source_ids, is_deleted=False).distinct()
    ...
    def get_object(self):
        ...
        try:
            ...
            self.check_object_permissions(self.request, obj)
            return obj
        except DocumentSource.DoesNotExist as e:
...
class FeedbackView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(serializer.to_representation(feedback), status=status.HTTP_201_CREATED)
