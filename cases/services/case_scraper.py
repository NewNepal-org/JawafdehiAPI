...
# NOTE: ``google-genai`` is a direct dependency (see pyproject); CaseScraper is its
# sole consumer. This module is imported eagerly at startup (cases.admin ->
# cases.services.__init__), but to keep the app bootable even if the Google SDK is
# absent (e.g. a stripped poller box), the Google imports are done lazily inside
# the methods that use them rather than at module load.
...
class Case(BaseModel):
    ...
    unverified_info: str = Field(
        default="",
        description="Unverified or incomplete information in rich HTML format",
    )
    ...
    sources: list[DocumentSource] = Field(
        description="Document sources supporting the case"
    )
