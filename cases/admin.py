...
class CaseAdminForm(forms.ModelForm):
    ...
    tags = MultiTextField(
        required=False, button_label="Add Tag", label="Tags", help_text="Tags for categorization"
    )
    ...
    evidence = MultiEvidenceField(
        required=False, label="Evidence", help_text="Evidence entries with source references"
    )
...
class CustomUserAdmin(BaseUserAdmin):
    ...
    def get_queryset(self, request):
        ...
        if is_moderator(request.user):
            ...
            moderator_group_users = User.objects.filter(groups__name="Moderator").values_list(
                "id", flat=True
            )
...
class FeedbackAdmin(admin.ModelAdmin):
    ...
    list_display = ["id", "feedback_type", "subject", "status", "has_contact_info", "submitted_at"]
