...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "source_paths", nargs="+", type=str, help="One or more source file paths to scrape"
        )
        parser.add_argument(
            "--language",
            type=str,
        ...
        parser.add_argument(
            "--location", type=str, default="us-central1", help="Google Cloud location"
        )
        parser.add_argument(
            "--model", type=str, default="gemini-2.5-pro", help="Google AI model to use"
        )
        parser.add_argument(
            "--create-db-entry",
            action="store_true",
    ...
    def import_to_database(
        self, json_file, case_data, no_confirm=False, case_type="CORRUPTION", case_state="DRAFT"
    ):
        """
        ...
        if not no_confirm:
            ...
            self.log_error(
                f"Related entities: {', '.join(case_data.related_entities) if case_data.related_entities else 'None'}"
            )
        ...
        try:
            ...
            raise CommandError(f"Failed to import case to database: {e}") from e
