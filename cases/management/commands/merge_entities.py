...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for entity_id in entity_ids:
            ...
            except JawafEntity.DoesNotExist as e:
                raise CommandError(f"Entity with ID {entity_id} does not exist") from e
        ...
        if len(all_nes_ids) > 1:
            ...
            self.stdout.write(self.style.ERROR(f"Only the first one will be kept: {merged_nes_id}"))
        ...
        if len(all_display_names) > 1:
            ...
            self.stdout.write(
                self.style.WARNING(f"Using display_name from target entity: {merged_display_name}")
            )
